...
// Difficulty Ranges matching build-time segmentation (kept for reference or backward compat)
export const EASY_MAX_INDEX = 1000;
export const MEDIUM_MAX_INDEX = 5000;
...
export function getGeneralIndices(difficulty: "easy" | "medium" | "hard" | "custom"): number[] {
  const maxIdx = difficulty === "easy" ? 1000 : difficulty === "medium" ? 5000 : WORDS.length;
  return Array.from({ length: maxIdx }, (_, i) => i);
}
